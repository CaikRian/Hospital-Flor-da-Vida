...
if (!isset($_SESSION['usuario'])) {
    header('Location: login_medico.php'); // Redireciona para o login se não estiver logado
    exit();
}

$nome_medico = $_SESSION['nome_usuario'];
$crm_medico = $_SESSION['usuario'];  // Obtém o CRM do médico da sessão

// Consulta para obter todas as consultas do médico, ordenadas pela data mais recente
$sqlConsultas = "
    SELECT consultas.data_consulta, consultas.horario_consulta, consultas.motivo_consulta, consultas.observacoes,
           pacientes.nome_completo AS paciente_nome, pacientes.cpf
    FROM consultas
    INNER JOIN pacientes ON consultas.id_paciente = pacientes.id
    INNER JOIN medicos ON consultas.id_medico = medicos.id
    WHERE medicos.crm = ?  
    ORDER BY consultas.data_consulta DESC"; // Ordena pela data mais recente

$stmtConsultas = $conn->prepare($sqlConsultas);
$stmtConsultas->bind_param("s", $crm_medico);  // Passa o CRM do médico como parâmetro
...
<head>
    ...
    <title>Consultas - Médico | Hospital Flor da Vida</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="../assets/css/style.css"> <!-- Importando o mesmo CSS do paciente -->
</head>
<body>
    <!-- Menu -->
    ...
    <div class="container mt-5 pt-5" id="custom_container">
        <div class="alert alert-primary text-center" role="alert">
            <h2>Bem-vindo, <?= htmlspecialchars($nome_medico); ?>!</h2>
            <p>Confira as consultas agendadas abaixo:</p>
        </div>

        <?php if (empty($consultas)): ?>
            <div class="alert alert-warning text-center">Você não tem consultas agendadas no momento.</div>
        <?php else: ?>
            <div class="row justify-content-center">
                <?php foreach ($consultas as $consulta): ?>
                    <div class="card m-3" style="max-width: 600px;">
                        <div class="card-header bg-primary text-white" style="border-radius: 0">
                            Consulta com <?= htmlspecialchars($consulta['paciente_nome']); ?>
                        </div>
                        <div class="card-body">
                            ...
                            <p><strong>Paciente:</strong> <?= htmlspecialchars($consulta['paciente_nome']); ?> (CPF: <?= htmlspecialchars($consulta['cpf']); ?>)</p>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
